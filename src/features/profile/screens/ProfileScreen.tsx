...
// --- NUEVA LÓGICA DE ASSETS PIXEL ---
// Generamos el mapeo de las 192 imágenes. 
// Nota: React Native requiere que los paths sean literales en el require.
const PIXEL_PORTRAITS: ImageSourcePropType[] = Array.from({ length: 192 }, (_, i) => {
  const id = i + 1;
  // Ajusta esta ruta si tu estructura de carpetas es diferente
  // asumiendo que el archivo está en: src/features/profile/screens/ProfileScreen.tsx
  // y las imágenes en: assets/pixel/portrait-with-border1.png
  switch(id) {
    case 1: return require("../../../../assets/avatars/pixel/portrait-with-border1.png");
    case 2: return require("../../../../assets/avatars/pixel/portrait-with-border2.png");
    case 3: return require("../../../../assets/avatars/pixel/portrait-with-border3.png");
    // ... Este bloque se debe completar hasta el 192. 
    // Por brevedad en la respuesta, aquí represento la lógica:
    default: return require("../../../../assets/avatars/pixel/portrait-with-border1.png"); 
  }
});

/**
 * IMPORTANTE: Para que lo anterior no sea un switch gigante manual, 
 * lo ideal es un script que genere este archivo de mapeo, 
 * pero aquí te dejo la estructura integrada.
 */
...
export function ProfileScreen({ navigation }: Props) {
  ...
  const loadProfile = useCallback(async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Ocurrio un error al cargar perfil.";
      setError(message);
    } finally {
      setIsLoadingProfile(false);
    }
  ...
  const handleSaveProfile = useCallback(async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Ocurrio un error al guardar perfil.";
      setError(message);
    } finally {
      setIsSaving(false);
    }
  ...
  const handleLogout = useCallback(async () => {
    try {
      ...
      const message = err instanceof Error ? err.message : "Ocurrió un error al cerrar sesión.";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleNextLocalAvatar = useCallback(() => {
    ...
    const options = LOCAL_AVATARS_BY_ROLE[roleKey];
    PIXEL_PORTRAITS
    const current = getLocalAvatarSelection(role, profileId || email || name, avatarUrl);
    const nextIndex = (current.index + 1) % options.length;
    
    setAvatarUrl(buildLocalAvatarToken(roleKey, nextIndex));
    setAvatarImageFailed(false);
    setSuccessMessage("Avatar pixel seleccionado. Guarda para confirmar.");
  }, [avatarUrl, email, name, profileId, role]);

  const trimmedAvatarUrl = useMemo(() => avatarUrl.trim(), [avatarUrl]);

  const localAvatar = useMemo(
    () => getLocalAvatarSelection(role, profileId || email || name, trimmedAvatarUrl),
    [role, profileId, email, name, trimmedAvatarUrl],
  );

  const shouldUseRemoteAvatar = useMemo(
    () => trimmedAvatarUrl.length > 0 && !avatarImageFailed && !isLocalAvatarToken(trimmedAvatarUrl),
    [trimmedAvatarUrl, avatarImageFailed],
  );
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <View style={styles.avatarSection}>
        ...
        <Text style={styles.avatarMetaText}>
          Avatar pixel {localAvatar.index + 1} de {localAvatar.total}
        </Text>
        <Text style={styles.profileName}>{name || "Sin nombre"}</Text>
        <Text style={styles.profileEmail}>{email}</Text>
        {activeGroupName ? (
      ...
      <View style={styles.card}>
        ...
        {canConfigureGroup ? (
          <View style={styles.actionSpacer}>
            <Button title="Configuracion del grupo" onPress={handleGoToGroupSettings} />
          </View>
        ) : null}
      </View>
...
/**
 * Distribuimos las 192 imágenes entre los roles.
 * Esto asegura que cada rol tenga su propio set de la carpeta 'pixel'.
 */
const LOCAL_AVATARS_BY_ROLE: Record<RoleKey, ImageSourcePropType[]> = {
  // Miembros: imágenes 1 a 64
  member: PIXEL_PORTRAITS.slice(0, 64),
  // Propietarios: imágenes 65 a 128
  owner: PIXEL_PORTRAITS.slice(64, 128),
  // Sub-propietarios: imágenes 129 a 192
  sub_owner: PIXEL_PORTRAITS.slice(128, 192),
};
...
function parseLocalAvatarToken(value: string): { role: RoleKey; index: number } | null {
  if (!isLocalAvatarToken(value)) return null;
  const parts = value.replace(LOCAL_AVATAR_TOKEN_PREFIX, "").split("/");
  if (parts.length !== 2) return null;
  const roleStr = parts[0];
  const index = Number(parts[1]);
  if (!Number.isInteger(index) || index < 0) return null;
  if (roleStr !== "member" && roleStr !== "owner" && roleStr !== "sub_owner") return null;
  return { role: roleStr as RoleKey, index };
}
...
function getLocalAvatarSelection(
  ...
  const roleKey = parsed !== null ? parsed.role : toRoleKey(role);
  const options = LOCAL_AVATARS_BY_ROLE[roleKey];
  const index = parsed !== null ? parsed.index % options.length : stableHash(userSeed) % options.length;
  
  return {
    source: options[index] || PIXEL_PORTRAITS[0],
    index,
    total: options.length,
  };
}
...
const styles = StyleSheet.create({
  container: { backgroundColor: colors.background, padding: 24, paddingBottom: 48 },
  ...
  avatarActionsRow: { marginBottom: 6 },
  avatarMetaText: { fontSize: 12, color: colors.muted, marginBottom: 8 },
  profileName: { fontSize: 22, fontWeight: "700", color: colors.text },
  profileEmail: { marginTop: 4, fontSize: 13, color: colors.muted },
  groupLabel: { marginTop: 6, fontSize: 13, color: colors.primary, fontWeight: "600" },
  statsRow: {
    flexDirection: "row",
    backgroundColor: colors.surface,
    borderWidth: 1,
  ...
  statLabel: { marginTop: 2, fontSize: 11, color: colors.muted },
  ...
  cardLabel: { fontSize: 13, color: colors.muted, marginBottom: 12 },
  input: {
    backgroundColor: colors.background,
    borderWidth: 1,
    borderColor: colors.border,
  ...
  successText: { color: "#0B6E4F", marginBottom: 10, fontSize: 13 },
  actionSpacer: { marginTop: 10 },
});